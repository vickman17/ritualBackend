import { IAnswerRepository } from '../repositories/interfaces/AnswerRepository.js';

export class AnswerService {
  constructor(private readonly answers: IAnswerRepository) {}

  async submit(roomId: number, questionId: number, userId: number, selectedIndex: number, elapsedMs?: number) {
    const correctIndex = await this.answers.getCorrectAnswerIndex(questionId, roomId);
    if (correctIndex === null) return { questionNotFound: true };
    const isCorrect = selectedIndex === correctIndex ? 1 : 0;
    const timing = await this.answers.getRoomTiming(roomId);
    if (!timing) return { roomNotFound: true };
  ...
  async leaderboard(roomId: number) {
  ...
  async myAnswers(roomId: number, userId: number) {
    return await this.answers.getMyAnswers(roomId, userId);
  }
  async myStatus(userId: number, roomIds: number[]) {
